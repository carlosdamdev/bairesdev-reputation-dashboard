...
<div class="page">
  ...
  <header class="header">
    <div class="header-left">
      ...
      <p>Weekly score monitoring across review platforms · 2026</p>
    </div>
  ...
  <div class="tab-pane active" id="platforms-pane">
    ...
    <div class="section-label">Current week indicators</div>
    <div class="kpi-grid" id="kpi-grid"></div>

    <div class="section-label">Overall evolution</div>
    <div class="chart-card">
      <div class="chart-card-header">
        <h2>Scores by platform</h2>
        <p>All tracked weeks</p>
      </div>
      <canvas id="overview-chart" height="85"></canvas>
    </div>

    <div class="section-label" style="margin-top:28px;">Evolution by platform</div>
    <div class="platform-grid" id="platform-grid"></div>

    <div class="section-label" style="margin-top:36px;">New reviews below average (last 3 weeks)</div>
    <div class="reviews-filters" id="reviews-filters"></div>
    <div class="reviews-grid" id="reviews-grid"></div>

    <div class="section-label" style="margin-top:48px;">Latest reviews — 4-week window</div>
  ...
  <div class="tab-pane" id="gmaps-pane">
    ...
    <div class="section-label">Current scores by office</div>
    <div class="kpi-grid kpi-grid--4" id="gmaps-kpi-grid"></div>

    <div class="section-label" style="margin-top:28px;">Weekly evolution by office</div>
    <div class="chart-card">
      <div class="chart-card-header">
        <h2>Google Maps — Score by week and office</h2>
        <p>Historical evolution · scale 1–5</p>
      </div>
      <canvas id="gmaps-chart" height="85"></canvas>
    </div>

    <div class="section-label" style="margin-top:36px;">Last week reviews with score &lt; 4.5</div>
...
document.getElementById('week-label').textContent =
  `Week ${D.currentWeek}  ·  ${fmt(D.currentDate)}`;
    ...
    <div class="kpi-card" style="--p-color:${color}">
      ...
      <div class="kpi-deltas">
        <div class="delta ${cls(dw)}">
          ...
          <strong>${dw !== 0 ? sign(dw)+Math.abs(dw).toFixed(2) : 'No change'}</strong>
          <span class="label">vs prev. week</span>
        </div>
        <div class="delta ${cls(dt)}">
          <span class="arrow">${arrow(dt)}</span>
          <strong>${dt !== 0 ? sign(dt)+Math.abs(dt).toFixed(2) : 'No change'}</strong>
          <span class="label">vs start of year</span>
        </div>
      </div>
    </div>`);
});
...
function buildReviewSection(reviews, filtersId, gridId, showThreshold) {
  ...
  const plats = ['All', ...new Set(reviews.map(r => r.plataforma))];
  ...
  plats.forEach(p => {
    ...
    btn.textContent = p;
    if (p !== 'All') btn.style.borderColor = D.colors[p] || '#444';
    btn.addEventListener('click', () => {
      activeFilter = p;
      filtersEl.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      gridEl.querySelectorAll('.review-card').forEach(card => {
        card.classList.toggle('hidden', activeFilter !== 'All' && card.dataset.plat !== activeFilter);
      });
    });
  ...
  reviews.forEach(r => {
    ...
    const footerExtra = showThreshold && r.threshold
      ? `<span style="color:var(--red)">★ ${r.rating.toFixed(1)}</span> <span style="color:var(--muted)">/ avg ${r.threshold.toFixed(1)}</span>`
...
function initGmapsTab() {
  ...
  if (!GM) {
    document.getElementById('gmaps-pane').insertAdjacentHTML('afterbegin',
      '<p style="color:var(--muted);padding:24px">No Google Maps data. Run gmaps_scraper.py first.</p>');
    return;
  }
  ...
  if (D.gmapsAlerts && D.gmapsAlerts.length) {
    ...
    document.getElementById('gmaps-alerts-grid').innerHTML =
      '<p style="color:var(--muted);font-size:0.8rem;padding:4px 0">No reviews below 4.5 this week.</p>';
  }
}
